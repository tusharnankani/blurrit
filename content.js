...
blurIt = () => {
  ...
  for (item of taggedNameList) {
    item.classList.add("blurrit-class", "blur-4");
  }

  for (i = 0; i < 256; ++i) {
    let potentialListOfUserNameColors = document.getElementsByClassName(
      `color-${i + 1}`
    );
    let potentialListOfUserNameBgColors = document.getElementsByClassName(
      `bg-color-${i + 1}`
    );
    for (item of potentialListOfUserNameColors) {
      item.classList.add("blurrit-class", "blur-4");
    }
    for (item of potentialListOfUserNameBgColors) {
      item.classList.add("blurrit-class", "blur-4");
    }
  }
